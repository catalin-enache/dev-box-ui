...
class Publisher {
  ...
  publish(channel, message) {
    ...
    this._subscribers[channel].forEach(subscriber => subscriber.receiveMessage(message));
  }
  ...
  subscribe(channel, subscriber, receiveLastMessage = true) {
    ...
    receiveLastMessage && currentMessage !== undefined && subscriber.receiveMessage(currentMessage);
    return () => {
      // unsubscribe
      this._subscribers[channel] = this._subscribers[channel].filter(sub => sub !== subscriber);
    };
  }
}
